...
Windows 本地答题纸工具与无 UI TypeScript SDK。已建立 M0 本地扫描原型，正在推进 M1 模板功能，尚未实现完整阅卷产品。
...
M1 模板核心位于 `src/AnswerSheet.Core`，无第三方包依赖。运行 `dotnet run --project tests/core/AnswerSheet.Core.Tests.csproj --no-restore -- --write-example` 可验证几何和 SVG 导出并生成本地示例；首次运行需恢复该测试项目。实施范围与验收条件见 [M1 计划](docs/architecture/m1.md)。M0 的 HTTPS 浏览器连接验证仍未完成，不能以本地健康检查替代。
